...
class Car {
  ...
  async loadCar(scene: THREE.Scene, position: [number, number, number]) {
    await new GLTFLoader().loadAsync('models/sedanSports.glb').then((gltf) => {
      ...
      const wheelBLMesh = gltf.scene.getObjectByName('wheel-back-left') as THREE.Group
      const wheelBRMesh = gltf.scene.getObjectByName('wheel-back-right') as THREE.Group
      const wheelFLMesh = gltf.scene.getObjectByName('wheel-front-left') as THREE.Group
      const wheelFRMesh = gltf.scene.getObjectByName('wheel-front-right') as THREE.Group
